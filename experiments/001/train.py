...
def train_single_fold(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if val_r2 > best_val_r2:
            ...
            logger.info(f"Saved best model (R²={val_r2:.5f})")
...
def train_single_run(
    ...
    train_df = create_folds(train_df, n_folds=n_folds, seed=seed)
    ...
    for fold in folds_to_train:
        ...
        logger.info(f"Fold {fold} - Best R²: {result['best_val_r2']:.5f} (epoch {result['best_epoch']})")
    ...
    with metrics_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["fold", "best_val_r2", "best_val_loss", "best_epoch"])
        for r in fold_results:
            writer.writerow([r["fold"], r["best_val_r2"], r["best_val_loss"], r["best_epoch"]])
...
def main() -> None:
    ...
    # Find best run
    best_summary = max(summaries, key=lambda x: x.get("avg_val_r2", -float("inf")))

    # Save summary
    summary_csv = experiment_dir / "summary.csv"
    if summaries:
        summaries_sorted = sorted(summaries, key=lambda x: -x.get("avg_val_r2", -float("inf")))

        fieldnames = ["avg_val_r2", "avg_val_loss", "run_name", "run_number"]
    ...
    print(f"Best run: {best_summary.get('run_name')} (R²={best_summary.get('avg_val_r2'):.5f})")
