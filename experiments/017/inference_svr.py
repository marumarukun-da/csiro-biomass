"""Inference script for SVR model with DINOv3 backbone feature extraction.

Supports:
- Multi-fold SVR ensemble
...
from src.svr_model import MultiTargetSVR
...
def load_svr_models(
    ...
    """Load SVR models from all folds.
    ...
    Returns:
        List of loaded SVR models
    """
    models = []

    for fold in folds:
        model_path = model_dir / "weights" / f"svr_fold{fold}.pkl"
        if not model_path.exists():
            raise FileNotFoundError(f"Model not found: {model_path}")

        model = MultiTargetSVR.load(model_path)
        models.append(model)
        print(f"Loaded SVR model from {model_path}")
...
def predict_single_image(
    image: np.ndarray,
    backbone: torch.nn.Module,
    svr_models: list[MultiTargetSVR],
    tta_transforms: list[tuple[str, A.Compose]],
    device: torch.device,
    img_size: int = 960,
) -> np.ndarray:
    """Predict for a single image using DINOv3 backbone and SVR models.

    Args:
        image: Original image [H, W, C] in RGB
        backbone: DINOv3 backbone for feature extraction
        svr_models: List of SVR models for fold ensemble
...
def run_inference(
    ...
    Args:
        ...
        backbone: DINOv3 backbone
        svr_models: List of SVR models for ensemble
    ...
    for image_path in tqdm(unique_images, desc="Inference"):
        ...
        pred_3 = predict_single_image(image, backbone, svr_models, tta_transforms, device, img_size)
...
def main():
    parser = argparse.ArgumentParser(description="SVR inference for biomass prediction")
    parser.add_argument(
        "--experiment_dir",
        type=str,
        required=True,
        help="Experiment directory name (e.g., 20241228_123456_exp014_dinov3_svr)",
    )
    parser.add_argument(
        "--folds",
        type=str,
    ...
    # Load SVR models
    svr_models = load_svr_models(model_dir, folds)
    print(f"Loaded {len(svr_models)} SVR models")
...
def kaggle_inference(
    ...
    # Load SVR models from artifact directory
