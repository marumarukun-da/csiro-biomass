...
if not config.IS_KAGGLE_ENV:
    ...
    # experiments/001/ ディレクトリのみ（プロジェクト全体ではない）
    dataset_upload(
...
**アップロードされる内容：**
- `csiro-biomass-artifacts`: モデル重み（`{run_name}/weights/`, `config.yaml`）
- `csiro-biomass-codes-001`: 実験001のコード（`config.py`, `inference.py`, `src/` 等）

**注意**: コードDataset名には実験名が含まれます（`csiro-biomass-codes-{EXP_NAME}`）。
これにより実験002, 003... と積み重ねても互いに上書きされません。
